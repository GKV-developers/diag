...
!-------------------------------------------------------------------------------
  ...
  public fluiddetailtrans_mxmyisloop
...
SUBROUTINE fluiddetailtrans_mxmyisloop( diag_mx, diag_my, is, loop )
!-------------------------------------------------------------------------------
!
!     Detailed triad transfer in fluid approximation
!                                                   (S. Maeyama, 8 July 2014)
...
!-------------------------------------------------------------------------------
  ...
  integer, intent(in) :: diag_mx, diag_my, is, loop 

  real(kind=DP) :: time
  complex(kind=DP), dimension(-nx:nx,-global_ny:global_ny,-global_nz:global_nz-1) :: phi, Al
  complex(kind=DP),  &
    dimension(-nx:nx,-global_ny:global_ny,-global_nz:global_nz-1,0:nmom-1) :: mom
  real(kind=DP) :: bb
  integer :: mx, my, iz, imom

!--- initialize ---
    call rb_phi_gettime( loop, time )
    call rb_phi_loop( loop, phi(:,0:global_ny,:) )
    call rb_Al_loop( loop, Al(:,0:global_ny,:) )
    call rb_mom_isloop( is, loop, mom(:,0:global_ny,:,:) )
    !- moments transform: gyrokinetic distribution -> non-adiabatic part
    do iz = -global_nz, global_nz-1
      do my = 0, global_ny 
        do mx = -nx, nx
          bb = gksq(mx,my,iz)*tau(is)*Anum(is)/(Znum(is)**2*gomg(iz)**2)
          mom(mx,my,iz,0) = mom(mx,my,iz,0)  &
                          + sgn(is) * fcs(is) * g0(mx,my,iz,is) * phi(mx,my,iz) / tau(is)
          mom(mx,my,iz,2) = mom(mx,my,iz,2)  &
                          + 0.5_DP * sgn(is) * fcs(is) * g0(mx,my,iz,is) * phi(mx,my,iz)
          mom(mx,my,iz,3) = mom(mx,my,iz,3)  &
                          + sgn(is) * fcs(is) * phi(mx,my,iz)  &
                          * ((1._DP - bb) * g0(mx,my,iz,is) + bb * g1(mx,my,iz,is))
        end do
      end do
    ...
    do iz = -global_nz, global_nz-1
      do my = 0, global_ny 
        do mx = -nx, nx
          mom(mx,my,iz,0) = Znum(is) * mom(mx,my,iz,0) / fcs(is)
          mom(mx,my,iz,1) = sqrt(Anum(is) / tau(is)) * Znum(is) * mom(mx,my,iz,1) / fcs(is)
          mom(mx,my,iz,2) = 2._DP * Znum(is) * mom(mx,my,iz,2) / (fcs(is) * tau(is)) - mom(mx,my,iz,0)
          mom(mx,my,iz,3) = - Znum(is) * mom(mx,my,iz,3) / (fcs(is) * tau(is)) + mom(mx,my,iz,0)
          mom(mx,my,iz,4) = 2._DP * sqrt(Anum(is) / tau(is)) * Znum(is) * mom(mx,my,iz,4)  &
                          / (fcs(is) * tau(is)) - 3._DP * mom(mx,my,iz,1)
          mom(mx,my,iz,5) = - sqrt(Anum(is) / tau(is)) * Znum(is) * mom(mx,my,iz,5)  &
                          / (fcs(is) * tau(is)) + mom(mx,my,iz,1)
        end do
      end do
    end do
    do iz = -global_nz, global_nz-1
    ...
    do imom = 0, nmom-1
      do iz = -global_nz, global_nz-1
        do my = 1, global_ny
          do mx = -nx, nx
            mom(-mx,-my,iz,imom) = conjg( mom(mx,my,iz,imom) )
          end do
        end do
      end do
    end do
...
!------------------
    ...
    call fluiddetailtrans_loop_calc( diag_mx, diag_my, is, loop, time, phi, Al, mom )


END SUBROUTINE fluiddetailtrans_mxmyisloop


SUBROUTINE fluiddetailtrans_loop_calc( diag_mx, diag_my, is, loop, time, phi, Al, mom )
!-------------------------------------------------------------------------------
!
!     Detailed triad transfer in fluid approximation
!                                                   (S. Maeyama, 8 July 2014)
...
!-------------------------------------------------------------------------------
  ...
  real(kind=DP), dimension(-nx:nx,-global_ny:global_ny,0:nmom-1) ::  &
                             jkpq_es, jpqk_es, jqkp_es, jkpq_em, jpqk_em, jqkp_em
  real(kind=DP) :: ceff, fct
  character(len=4) :: cmx, cmy
  character(len=8) :: cloop
  character(len=1) :: cis
  integer :: mx, my, px, py, qx, qy, iz, imom
    ...
    do imom = 0, nmom-1
      ...
      do iz = -global_nz, global_nz-1
        ...
        do py = max(-global_ny-my,-global_ny), min(global_ny,global_ny-my)
          ...
          do px = max(-nx-mx,-nx), min(nx,nx-mx)
            qx = - px - mx
            jkpq_es(px,py,imom) = jkpq_es(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(px) * wky(qy) + wky(py) * kx(qx))        &
                                  * real( (   phi(px,py,iz) * mom(qx,qy,iz,imom)     &
                                            - phi(qx,qy,iz) * mom(px,py,iz,imom) )   &
                                            * mom(mx,my,iz,imom), kind=DP ) )
            jpqk_es(px,py,imom) = jpqk_es(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(qx) * wky(my) + wky(qy) * kx(mx))        &
                                  * real( (   phi(qx,qy,iz) * mom(mx,my,iz,imom)     &
                                            - phi(mx,my,iz) * mom(qx,qy,iz,imom) )   &
                                            * mom(px,py,iz,imom), kind=DP ) )
            jqkp_es(px,py,imom) = jqkp_es(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(mx) * wky(py) + wky(my) * kx(px))        &
                                  * real( (   phi(mx,my,iz) * mom(px,py,iz,imom)     &
                                            - phi(px,py,iz) * mom(mx,my,iz,imom) )   &
                                            * mom(qx,qy,iz,imom), kind=DP ) )
          end do
        end do
      end do
    ...
    imom = 0
      ...
      do iz = -global_nz, global_nz-1
        ...
        do py = max(-global_ny-my,-global_ny), min(global_ny,global_ny-my)
          ...
          do px = max(-nx-mx,-nx), min(nx,nx-mx)
            qx = - px - mx
            jkpq_em(px,py,imom) = jkpq_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(px) * wky(qy) + wky(py) * kx(qx))  &
                      * real(   Al(px,py,iz) * mom(qx,qy,iz,0) * mom(mx,my,iz,1)  &
                              - Al(qx,qy,iz) * mom(px,py,iz,0) * mom(mx,my,iz,1)  &
                              + Al(px,py,iz) * mom(qx,qy,iz,1) * mom(mx,my,iz,0)  &
                              - Al(qx,qy,iz) * mom(px,py,iz,1) * mom(mx,my,iz,0), kind=DP ) )
            jpqk_em(px,py,imom) = jpqk_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(qx) * wky(my) + wky(qy) * kx(mx))  &
                      * real(   Al(qx,qy,iz) * mom(mx,my,iz,0) * mom(px,py,iz,1)  &
                              - Al(mx,my,iz) * mom(qx,qy,iz,0) * mom(px,py,iz,1)  &
                              + Al(qx,qy,iz) * mom(mx,my,iz,1) * mom(px,py,iz,0)  &
                              - Al(mx,my,iz) * mom(qx,qy,iz,1) * mom(px,py,iz,0), kind=DP ) )
            jqkp_em(px,py,imom) = jqkp_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(mx) * wky(py) + wky(my) * kx(px))  &
                      * real(   Al(mx,my,iz) * mom(px,py,iz,0) * mom(qx,qy,iz,1)  &
                              - Al(px,py,iz) * mom(mx,my,iz,0) * mom(qx,qy,iz,1)  &
                              + Al(mx,my,iz) * mom(px,py,iz,1) * mom(qx,qy,iz,0)  &
                              - Al(px,py,iz) * mom(mx,my,iz,1) * mom(qx,qy,iz,0), kind=DP ) )
          end do
        end do
      end do
    imom = 1
      ...
      do iz = -global_nz, global_nz-1
        ...
        do py = max(-global_ny-my,-global_ny), min(global_ny,global_ny-my)
          ...
          do px = max(-nx-mx,-nx), min(nx,nx-mx)
            ...
            jkpq_em(px,py,imom) = jkpq_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(px) * wky(qy) + wky(py) * kx(qx))  &
                      * real(   Al(px,py,iz) * mom(qx,qy,iz,1) * mom(mx,my,iz,2)  &
                              - Al(qx,qy,iz) * mom(px,py,iz,1) * mom(mx,my,iz,2)  &
                              + Al(px,py,iz) * mom(qx,qy,iz,2) * mom(mx,my,iz,1)  &
                              - Al(qx,qy,iz) * mom(px,py,iz,2) * mom(mx,my,iz,1), kind=DP ) )
            jpqk_em(px,py,imom) = jpqk_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(qx) * wky(my) + wky(qy) * kx(mx))  &
                      * real(   Al(qx,qy,iz) * mom(mx,my,iz,1) * mom(px,py,iz,2)  &
                              - Al(mx,my,iz) * mom(qx,qy,iz,1) * mom(px,py,iz,2)  &
                              + Al(qx,qy,iz) * mom(mx,my,iz,2) * mom(px,py,iz,1)  &
                              - Al(mx,my,iz) * mom(qx,qy,iz,2) * mom(px,py,iz,1), kind=DP ) )
            jqkp_em(px,py,imom) = jqkp_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(mx) * wky(py) + wky(my) * kx(px))  &
                      * real(   Al(mx,my,iz) * mom(px,py,iz,1) * mom(qx,qy,iz,2)  &
                              - Al(px,py,iz) * mom(mx,my,iz,1) * mom(qx,qy,iz,2)  &
                              + Al(mx,my,iz) * mom(px,py,iz,2) * mom(qx,qy,iz,1)  &
                              - Al(px,py,iz) * mom(mx,my,iz,2) * mom(qx,qy,iz,1), kind=DP ) )
          end do
        end do
      end do
    imom = 2
      ...
      do iz = -global_nz, global_nz-1
        ...
        do py = max(-global_ny-my,-global_ny), min(global_ny,global_ny-my)
          ...
          do px = max(-nx-mx,-nx), min(nx,nx-mx)
            ...
            jkpq_em(px,py,imom) = jkpq_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(px) * wky(qy) + wky(py) * kx(qx))  &
                      * real(   Al(px,py,iz) * mom(qx,qy,iz,2) * mom(mx,my,iz,4)  &
                              - Al(qx,qy,iz) * mom(px,py,iz,2) * mom(mx,my,iz,4)  &
                              + Al(px,py,iz) * mom(qx,qy,iz,4) * mom(mx,my,iz,2)  &
                              - Al(qx,qy,iz) * mom(px,py,iz,4) * mom(mx,my,iz,2), kind=DP ) )
            jpqk_em(px,py,imom) = jpqk_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(qx) * wky(my) + wky(qy) * kx(mx))  &
                      * real(   Al(qx,qy,iz) * mom(mx,my,iz,2) * mom(px,py,iz,4)  &
                              - Al(mx,my,iz) * mom(qx,qy,iz,2) * mom(px,py,iz,4)  &
                              + Al(qx,qy,iz) * mom(mx,my,iz,4) * mom(px,py,iz,2)  &
                              - Al(mx,my,iz) * mom(qx,qy,iz,4) * mom(px,py,iz,2), kind=DP ) )
            jqkp_em(px,py,imom) = jqkp_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(mx) * wky(py) + wky(my) * kx(px))  &
                      * real(   Al(mx,my,iz) * mom(px,py,iz,2) * mom(qx,qy,iz,4)  &
                              - Al(px,py,iz) * mom(mx,my,iz,2) * mom(qx,qy,iz,4)  &
                              + Al(mx,my,iz) * mom(px,py,iz,4) * mom(qx,qy,iz,2)  &
                              - Al(px,py,iz) * mom(mx,my,iz,4) * mom(qx,qy,iz,2), kind=DP ) )
          end do
        end do
      end do
    imom = 3
      ...
      do iz = -global_nz, global_nz-1
        ...
        do py = max(-global_ny-my,-global_ny), min(global_ny,global_ny-my)
          ...
          do px = max(-nx-mx,-nx), min(nx,nx-mx)
            ...
            jkpq_em(px,py,imom) = jkpq_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(px) * wky(qy) + wky(py) * kx(qx))  &
                      * real(   Al(px,py,iz) * mom(qx,qy,iz,3) * mom(mx,my,iz,5)  &
                              - Al(qx,qy,iz) * mom(px,py,iz,3) * mom(mx,my,iz,5)  &
                              + Al(px,py,iz) * mom(qx,qy,iz,5) * mom(mx,my,iz,3)  &
                              - Al(qx,qy,iz) * mom(px,py,iz,5) * mom(mx,my,iz,3), kind=DP ) )
            jpqk_em(px,py,imom) = jpqk_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(qx) * wky(my) + wky(qy) * kx(mx))  &
                      * real(   Al(qx,qy,iz) * mom(mx,my,iz,3) * mom(px,py,iz,5)  &
                              - Al(mx,my,iz) * mom(qx,qy,iz,3) * mom(px,py,iz,5)  &
                              + Al(qx,qy,iz) * mom(mx,my,iz,5) * mom(px,py,iz,3)  &
                              - Al(mx,my,iz) * mom(qx,qy,iz,5) * mom(px,py,iz,3), kind=DP ) )
            jqkp_em(px,py,imom) = jqkp_em(px,py,imom) + fct * (  &! flux-surface average
                                - ceff * (- kx(mx) * wky(py) + wky(my) * kx(px))  &
                      * real(   Al(mx,my,iz) * mom(px,py,iz,3) * mom(qx,qy,iz,5)  &
                              - Al(px,py,iz) * mom(mx,my,iz,3) * mom(qx,qy,iz,5)  &
                              + Al(mx,my,iz) * mom(px,py,iz,5) * mom(qx,qy,iz,3)  &
                              - Al(px,py,iz) * mom(mx,my,iz,5) * mom(qx,qy,iz,3), kind=DP ) )
          end do
        end do
      end do
!----------------------------
    ...
    write( cloop, fmt="(i8.8)" ) loop
!--- ascii output ---
    open( ofldtrninkxky, file="./data/fluiddetailtransinkxky_x"//cmx//"y"//cmy//"s"//cis//"_t"//cloop//".dat" )
      write( ofldtrninkxky, "(a,i17,a,g17.7e3)" ) "# loop=",loop,", time=",time
      write( ofldtrninkxky, "(a,i17,a,g17.7e3)" ) "#   mx=",mx,  ",   kx=",kx(mx)
      write( ofldtrninkxky, "(a,i17,a,g17.7e3)" ) "#  gmy=",my,  ",   ky=",gky(my)
      write( ofldtrninkxky, "(99a17)" ) "#          [1] px", "[2] py",  &
                                        "[3] jkpq_es", "[4] jpqk_es", "[5] jqkp_es", &
                                        "[6] jkpq_em", "[7] jpqk_em", "[8] jqkp_em"
      do py = -global_ny, global_ny
        do px = -nx, nx
          write( ofldtrninkxky, "(99g17.7e3)" ) kx(px), wky(py),        &
                                                sum(jkpq_es(px,py,:)),  &
                                                sum(jpqk_es(px,py,:)),  &
                                                sum(jqkp_es(px,py,:)),  &
                                                sum(jkpq_em(px,py,:)),  &
