...
SUBROUTINE fluidtotaltrans_isloop( is, loop )
!-------------------------------------------------------------------------------
!
!     Total triad transfer in fluid approximation
!                                                   (S. Maeyama, 8 July 2014)
...
!-------------------------------------------------------------------------------
  ...
  real(kind=DP), dimension(0:2*nxw-1,0:2*nyw-1) :: wkxy
  real(kind=DP), dimension(0:2*nxw-1,0:2*nyw-1,-global_nz:global_nz-1) :: dpdx, dpdy
  real(kind=DP), dimension(0:2*nxw-1,0:2*nyw-1,-global_nz:global_nz-1,0:nmom-1) :: dfdx, dfdy
  real(kind=DP), dimension(-nx:nx,0:global_ny,0:nmom-1) :: tk_es, tk_em
  real(kind=DP) :: bb, fct
  integer :: ix, iy, mx, my, iz, imom
...
!--- calc. total transfer ---
    do imom = 0, nmom-1
      do iz = -global_nz, global_nz-1
        do my = 0, global_ny
          do mx = -nx, nx
            ikxf(mx,my) = ui * kx(mx) * mom(mx,my,iz,imom)
            ikyf(mx,my) = ui * gky(my) * mom(mx,my,iz,imom)
          end do
        end do
        call fft_backward_xy(ikxf, dfdx(:,:,iz,imom))
        call fft_backward_xy(ikyf, dfdy(:,:,iz,imom))
      end do
    end do
    ...
    do imom = 0, nmom-1
      ...
      if ( imom == 0 .or. imom == 1 .or. imom == 3 .or. imom == 5 ) then
        do iz = -global_nz, global_nz-1
          ...
          do iy = 0, 2*nyw-1
            do ix = 0, 2*nxw-1
              wkxy(ix,iy) = - dpdx(ix,iy,iz) * dfdy(ix,iy,iz,imom)  &
                            + dpdy(ix,iy,iz) * dfdx(ix,iy,iz,imom)
            end do
          end do
          call fft_forward_xy(wkxy, nf)
          do my = 0, global_ny
            do mx = -nx, nx
              tk_es(mx,my,imom) = tk_es(mx,my,imom) + fct * (  &! flux-surface average
                                  (fcs(is) * tau(is) / Znum(is))  &
                                * real( conjg(mom(mx,my,iz,imom)) * nf(mx,my), kind=DP ) )
            end do
          end do
        end do
      else if ( imom == 2 ) then
    ...
    do iz = -global_nz, global_nz-1
      ...
      imom = 0
        do iy = 0, 2*nyw-1
          do ix = 0, 2*nxw-1
            wkxy(ix,iy) = - dpdx(ix,iy,iz) * dfdy(ix,iy,iz,0)  &
                          + dpdy(ix,iy,iz) * dfdx(ix,iy,iz,0)
          end do
        end do
        call fft_forward_xy(wkxy, nf)
        do my = 0, global_ny
          do mx = -nx, nx
            tk_em(mx,my,imom) = tk_em(mx,my,imom) + fct * (  &! flux-surface average
                                (fcs(is) * tau(is) / Znum(is))  &
                              * (- sqrt(tau(is) / Anum(is)))    &
                              * real( conjg(mom(mx,my,iz,1)) * nf(mx,my), kind=DP ) )
          end do
        end do
        do iy = 0, 2*nyw-1
          do ix = 0, 2*nxw-1
            wkxy(ix,iy) = - dpdx(ix,iy,iz) * dfdy(ix,iy,iz,1)  &
                          + dpdy(ix,iy,iz) * dfdx(ix,iy,iz,1)
          end do
        end do
        call fft_forward_xy(wkxy, nf)
        do my = 0, global_ny
          do mx = -nx, nx
            tk_em(mx,my,imom) = tk_em(mx,my,imom) + fct * (  &! flux-surface average
                                (fcs(is) * tau(is) / Znum(is))  &
                              * (- sqrt(tau(is) / Anum(is)))    &
                              * real( conjg(mom(mx,my,iz,0)) * nf(mx,my), kind=DP ) )
          end do
        end do
      imom = 1
        !do iy = 0, 2*nyw-1
        ...
        do my = 0, global_ny
          do mx = -nx, nx
            tk_em(mx,my,imom) = tk_em(mx,my,imom) + fct * (  &! flux-surface average
                                (fcs(is) * tau(is) / Znum(is))  &
                              * (- sqrt(tau(is) / Anum(is)))    &
                              * real( conjg(mom(mx,my,iz,2)) * nf(mx,my), kind=DP ) )
          end do
        end do
        do iy = 0, 2*nyw-1
          do ix = 0, 2*nxw-1
            wkxy(ix,iy) = - dpdx(ix,iy,iz) * dfdy(ix,iy,iz,2)  &
                          + dpdy(ix,iy,iz) * dfdx(ix,iy,iz,2)
          end do
        end do
        call fft_forward_xy(wkxy, nf)
        do my = 0, global_ny
        ...
        do my = 0, global_ny
          do mx = -nx, nx
            tk_em(mx,my,imom) = tk_em(mx,my,imom) + fct * (  &! flux-surface average
                                (fcs(is) * tau(is) / Znum(is)) * 0.5_DP  &
                              * (- sqrt(tau(is) / Anum(is)))    &
                              * real( conjg(mom(mx,my,iz,4)) * nf(mx,my), kind=DP ) )
          end do
        end do
        do iy = 0, 2*nyw-1
          do ix = 0, 2*nxw-1
            wkxy(ix,iy) = - dpdx(ix,iy,iz) * dfdy(ix,iy,iz,4)  &
                          + dpdy(ix,iy,iz) * dfdx(ix,iy,iz,4)
          end do
        end do
        call fft_forward_xy(wkxy, nf)
        do my = 0, global_ny
      ...
      imom = 3
        do iy = 0, 2*nyw-1
          do ix = 0, 2*nxw-1
            wkxy(ix,iy) = - dpdx(ix,iy,iz) * dfdy(ix,iy,iz,3)  &
                          + dpdy(ix,iy,iz) * dfdx(ix,iy,iz,3)
          end do
        end do
        call fft_forward_xy(wkxy, nf)
        do my = 0, global_ny
          do mx = -nx, nx
            tk_em(mx,my,imom) = tk_em(mx,my,imom) + fct * (  &! flux-surface average
                                (fcs(is) * tau(is) / Znum(is))  &
                              * (- sqrt(tau(is) / Anum(is)))    &
                              * real( conjg(mom(mx,my,iz,5)) * nf(mx,my), kind=DP ) )
          end do
        end do
        do iy = 0, 2*nyw-1
          do ix = 0, 2*nxw-1
            wkxy(ix,iy) = - dpdx(ix,iy,iz) * dfdy(ix,iy,iz,5)  &
                          + dpdy(ix,iy,iz) * dfdx(ix,iy,iz,5)
          end do
        end do
        call fft_forward_xy(wkxy, nf)
        do my = 0, global_ny
          do mx = -nx, nx
            tk_em(mx,my,imom) = tk_em(mx,my,imom) + fct * (  &! flux-surface average
                                (fcs(is) * tau(is) / Znum(is))  &
                              * (- sqrt(tau(is) / Anum(is)))    &
                              * real( conjg(mom(mx,my,iz,3)) * nf(mx,my), kind=DP ) )
          end do
        end do
...
!--- ascii output ---
    open( oflttrninkxky, file="./data/fluidtotaltransinkxky_s"//cis//"_t"//cloop//".dat" )
      write( oflttrninkxky, "(a,i17,a,g17.7e3)" ) "# loop=",loop, ", time=",time
      write( oflttrninkxky, "(99a17)" ) "#              kx","ky","tk_es","tk_em"
      do my = 0, global_ny
        do mx = -nx, nx
          write( oflttrninkxky, "(99g17.7e3)" ) kx(mx), gky(my), sum(tk_es(mx,my,:)), sum(tk_em(mx,my,:))
        end do
        write( oflttrninkxky, * )
      end do
    close( oflttrninkxky )
...
END SUBROUTINE fluidtotaltrans_isloop
